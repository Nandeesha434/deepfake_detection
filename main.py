...
def train_model(args):
    ...
    if args.data_type == 'images':
        X, y = builder.load_from_images(config.REAL_PATH, config.FAKE_PATH)
    else:
        X, y = builder.load_from_videos(config.REAL_PATH, config.FAKE_PATH, 
    ...
    trainer = DeepfakeTrainer(config, model_name=args.model)
